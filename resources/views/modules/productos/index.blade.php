        ...
        <div class="card shadow-lg border-0 rounded-3">
          <div class="card-body">
            ...
            <div class="card-header bg-gradient bg-primary text-white d-flex justify-content-between align-items-center rounded-top">
              ...
              <a href="{{ route('productos.create') }}" class="btn btn-light text-primary fw-bold shadow-sm">
            ...
            <form method="GET" action="{{ route('productos.index') }}" class="mb-4 mt-3">
              <div class="input-group shadow-sm">
                <input type="text" name="buscar" class="form-control border-primary" placeholder="🔍 Buscar producto..." value="{{ $query ?? '' }}">
                ...
                <div class="col-md-4 mb-4">
                  <div class="card h-100 shadow-sm border-0 rounded-3 hover-card">
                    ...
                    <div class="card-body">
                      ...
                      <p class="mb-1"><strong>Precio compra:</strong> ${{ number_format($producto->precio_compra, 2) }}</p>
                      <p class="mb-1"><strong>Precio venta:</strong> ${{ number_format($producto->precio_venta, 2) }}</p>
                    </div>
                    ...
                    <div class="card-footer d-flex flex-column gap-2 bg-light rounded-bottom">
                      ...
                      <form action="{{ route('carrito.store') }}" method="POST" class="d-flex align-items-center">
                        ...
                        <input type="number" name="cantidad" value="1" min="1"
                               class="form-control me-2 border-primary shadow-sm" style="width: 80px;">
                      ...
                      <div class="d-flex gap-2">
                        <a href="{{ route('productos.edit', $producto->id_producto) }}" class="btn btn-primary btn-sm fw-bold shadow-sm">
                          ✏️ Editar
                        </a>
                        <form method="POST" action="{{ route('productos.destroy', $producto->id_producto) }}">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="btn btn-danger btn-sm fw-bold shadow-sm" onclick="return confirm('¿Eliminar producto?')">
                            🗑️ Eliminar
                          </button>
                        </form>
                      </div>
